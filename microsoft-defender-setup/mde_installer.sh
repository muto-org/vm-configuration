...
#============================================================================
# https://raw.githubusercontent.com/microsoft/mdatp-xplat/master/linux/installation/mde_installer.sh
...
print_state()
{
    if [ -z $(which mdatp) ]; then
        ...
        if run_quietly "mdatp health" "[S] Could not connect to the daemon -- MDE is not ready to connect yet."; then
            ...
            log_info "[S] Real-time protection: $(mdatp health --field real_time_protection_enabled)"
...
set_real_time_protection()
{
    ...
    if [[ $(mdatp health --field real_time_protection_enabled | tail -1) == "false" ]]; then
        log_info "[>] setting MDE/EPP to real-time protection"
        retry_quietly 3 "mdatp config real-time-protection --value enabled" "failed to set MDE to real-time protection" $ERR_PARAMETER_SET_FAILED
    else
        log_info "[>] MDE/EPP already in real-time protection"
    fi

    log_info "[v] real-time protection set"
}
...
usage()
{
    ...
    echo " --real-time-protecton set real-time protection"
...
do
    case "$1" in
        ...
        --real-time-protection)
            verify_privileges "real-time-protection"
...
if [ ! -z $REAL_TIME_PROTECTION ]; then
    set_real_time_protection
fi
